...
public class StateTransitionCoordinationBenchmark
{
	...
	private IStateTransitionCoordination _zeroAllocCoordination = null!;
	...
	public void Setup()
	{
		...
		{
			_standardCoordination.AddAnyTransition(new StateTransition(new EmptyState(), () => false));
		}
		...
		_zeroAllocCoordination = new StateTransitionCoordination.ZeroAlloc();
		for (var i = 0; i < ANY_TRANSITIONS_COUNT; i++)
		{
			_zeroAllocCoordination.AddAnyTransition(new StateTransition.ZeroAlloc(new EmptyState(), () => false));
		}

		for (var i = 0; i < CURRENT_TRANSITIONS_COUNT; i++)
		{
			_zeroAllocCoordination.AddTransition(new EmptyState(),
				new StateTransition.ZeroAlloc(new EmptyState(), () => false));
		}
	}
